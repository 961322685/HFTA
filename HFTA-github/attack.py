...
parser.add_argument('--save_path', type=str, default='manifold/image_noise/', help='Save images.')
...
def graph(x, model, x_min, x_max):
    ...
    for i in range(num_iter):
        ...
        #loss = 1 - cos(model(x_ori), model(adv))
        loss = torch.dist(model(x_ori), model(x_noise), p=2)
        loss.backward()
        noise = x.grad.data
...
def main():
    ...
    for images, _ in dataloader:
        ...
        adv_img = graph(images, model, images_min, images_max)
        # print(adv_img)
