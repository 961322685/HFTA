...
import yaml
sys.path.append('../../')
from facex_net.ResNets import Resnet
from facex_net.MobileFaceNets import MobileFaceNet
from facex_net.MobileFaceNets_n import MobileFaceNet_n
from facex_net.EfficientNets import EfficientNet
from facex_net.EfficientNets import efficientnet
from facex_net.HRNet import HighResolutionNet
from facex_net.GhostNet import GhostNet
from facex_net.AttentionNets import ResidualAttentionNet
from facex_net.TF_NAS import TF_NAS_A
from facex_net.resnest.resnest import ResNeSt
from facex_net.ReXNets import ReXNetV1
from facex_net.RepVGG import RepVGG
...
class BackboneFactory:
    ...
    def get_backbone(self):
        ...
        elif self.backbone_type == 'MobileFaceNet_n':
            feat_dim = self.backbone_param['feat_dim']  # dimension of the output features, e.g. 512.
            out_h = self.backbone_param['out_h']  # height of the feature map before the final features.
            out_w = self.backbone_param['out_w']  # width of the feature map before the final features.
            backbone = MobileFaceNet_n(feat_dim, out_h, out_w)
        elif self.backbone_type == 'Resnet50-ir':
        ...
        elif self.backbone_type == 'Resnet152-irse':
            depth = self.backbone_param['depth'] # depth of the ResNet, e.g. 50, 100, 152.
            drop_ratio = self.backbone_param['drop_ratio'] # drop out ratio.
            net_mode = self.backbone_param['net_mode'] # 'ir' for improved by resnt, 'ir_se' for SE-ResNet.
            feat_dim = self.backbone_param['feat_dim'] # dimension of the output features, e.g. 512.
